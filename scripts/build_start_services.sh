...
if [ $? -eq 0 ]; then
    echo "   ${GREEN}静态文件收集成功${NC}"
else
    echo "   ${YELLOW}静态文件收集失败，继续启动${NC}"
fi
sleep 1

# 3. 启动后端（使用 Gunicorn）
echo "3. 启动 Django 后端服务 (Gunicorn)..."
if check_running "gunicorn.*xxm_fans_home.wsgi"; then
    ...
    echo "   ${GREEN}Django 后端启动成功 (端口 8000)${NC}"
fi
sleep 2
...
if check_running "nginx.*prod-xxm_nginx.conf"; then
    ...
    echo "   ${GREEN}Nginx 启动成功 (端口 8080)${NC}"
fi
